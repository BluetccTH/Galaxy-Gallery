  ...
  const [config, setConfig] = useState<GalaxyConfig>(() => {
    ...
    if (saved) {
      try {
        ...
        if (parsed) {
          if (parsed.pageName === "Galaxy Gallery" || parsed.pageName?.toLowerCase().includes("galaxy") || !parsed.pageName) {
            parsed.pageName = "ICE";
          }
...
ด้วยความรักและความจริงใจจากหัวใจของหนู 🤍`.split("\n\n").map((paragraph, index) => (
